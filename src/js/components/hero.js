...
export function pushFetch() {
  try {
    ...
    return response.then(({ data }) => {
      markUp(data);
      hideSpinner();
      axios.get(`${URL}trending/movie/day?api_key=${API_KEY}&page=${pageNum}&language=uk`).then(({ data }) => { localStorage.setItem('data-ua', `${JSON.stringify(data)}`) });
      axios.get(`${URL}trending/movie/day?api_key=${API_KEY}&page=${pageNum}&language=en`).then(({ data }) => { localStorage.setItem('data-en', `${JSON.stringify(data)}`) });
      axios.get(`${URL}trending/movie/day?api_key=${API_KEY}&page=${pageNum}&language=pl`).then(({ data }) => { localStorage.setItem('data-pl', `${JSON.stringify(data)}`) });  
    });
  } catch (error) {
    hideSpinner();
    console.log(error);
